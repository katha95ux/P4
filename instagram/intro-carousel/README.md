...
tiny doodles → lots of breathing room.
    ...
    slides/slide4.jpg   so… why am I here?          (bouquet — light slide)
...
| file | what it does |
| --- | --- |
| `build/prep.py` | crops each photo to 4:5, extends the canvas where a slide needs clean room for text, applies the warm film grade (lifted blacks, warm highlights, grain, per-slide vignette) |
...
- On the three dark slides: cream `#FCF7EC` ink, gold `#F2C86E` accents
- On slide 4 (light photo): warm brown `#3E2C22` ink, ochre `#A8710F`
  accents, and a light scrim instead of a dark one
